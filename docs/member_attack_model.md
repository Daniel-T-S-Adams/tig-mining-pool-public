...
| M20 | Deliberately fail a benchmark so the uncovered charge falls on other members | Innocent earners in that round lose `(10_000 - M_bps)/10_000` of the penalty, pro-rata to what they earned | **Leverage is `(10_000 - M_bps) / M_bps`**: the attacker forfeits their reservation to impose that multiple on others — 1:1 at `5_000` bps, 9:1 at `1_000`, 99:1 at `100`. It is not profitable, since failed work earns nothing, so the motive is griefing or competitive damage rather than gain. The channel exists only below `10_000` bps and only for members the pool chose to discount, so **the pool grants the leverage to the members it trusts most** — the control is the multiplier policy and §13 item 21's reported aggregate, not a runtime check. Amplifies M18: coordinated low-multiplier accounts multiply the same leverage (`accounting.md` §8.4, ADR 0014) |
