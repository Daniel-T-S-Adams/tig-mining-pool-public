...
Live observations from the verification call, for S1's assumption checks:
block IDs are opaque 32-hex strings (not height-derived); the
`get-player-data` envelope carries additional top-level keys (`deposits`,
`round_earnings`) beside `player`.
