...
| Suspend or reinstate a member account | Controller | Operator-command ID, actor and reason; the Pool API creates accounts and cannot change their standing. Reinstating a worker revoked as a security action (`member_protocol.md` §3.3) belongs to this owner and is **refused by the schema until the operator-command path exists**, rather than left to whoever holds an UPDATE |
